...
const CameraConfigPanel = ({ 
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      // Mock test result based on IP validity
      const isValidIP = /^(\d{1,3}\.){3}\d{1,3}$/?.test(formData?.ipAddress);
      const result = {
        success: isValidIP && formData?.ipAddress !== '192.168.1.999',
        latency: isValidIP ? Math.floor(Math.random() * 50) + 10 : null,
        resolution: isValidIP ? formData?.resolution : null,
        message: isValidIP 
          ? 'Connection successful' :'Invalid IP address or camera unreachable'
      };
